...
def get_active_window_mmsg():
    try:
        output = subprocess.check_output(['mmsg', 'get', 'activewindow', '-j'], text=True)
        if output.strip() == "{}": return "Desktop", "Desktop"
        data = json.loads(output)
        
        app_cls = data.get('appid') or data.get('class') or ''
...
def main():
    ...
    current_app_class, current_app_title = get_active_window_mmsg()
    
    ipc_thread = threading.Thread(target=listen_mango_ipc, daemon=True)
    ipc_thread.start()
