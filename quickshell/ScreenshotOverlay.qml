...
PanelWindow {
    ...
    function performQrScan() {
        ...
        let cmd = `bash ~/.local/share/mango/scripts/screenshot.sh --geometry "${root.geometryString}" --scan-qr`
    ...
    function executeCapture(openEditor, isRecord) {
        let cmd = `bash ~/.local/share/mango/scripts/screenshot.sh --geometry "${root.geometryString}"`
        if (isRecord) {
            cmd += " --record"
